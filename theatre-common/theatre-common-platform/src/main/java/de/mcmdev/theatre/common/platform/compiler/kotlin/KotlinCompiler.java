package de.mcmdev.theatre.common.platform.compiler.kotlin;
...
public class KotlinCompiler extends ScriptCompiler {
    public KotlinCompiler(File compiledDirectory, List<String> classpath, ScriptType scriptType) {
    ...
    public CompiledScript compile(ScriptSource scriptSource) throws CompilationException {
        ...
        try {
            Process process = new ProcessBuilder("./kotlin-compiler/bin/kotlinc", "-d", compiledDirectory.getAbsolutePath(), "-cp", buildClasspathString(), absolutePath).start();
